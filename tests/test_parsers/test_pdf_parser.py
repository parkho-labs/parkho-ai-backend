...
class TestPDFParser:
    ...
    @pytest.mark.asyncio
    async def test_parse_valid_pdf(self, mock_exists, mock_getsize, mock_markdown):
    ...
    @patch('os.path.exists')
    @pytest.mark.asyncio
    async def test_parse_empty_content(self, mock_exists, mock_getsize, mock_markdown):
        mock_exists.return_value = True
        mock_getsize.return_value = 1024
    ...
    def test_build_metadata(self):
        content = "Page 1\n\n-----\n\nPage 2"
        metadata = self.parser._build_metadata("/test/document.pdf", 1024, content)
